...
class RTKManager:
    ...
    def stop_service(self) -> bool:
        ...
        if thread and thread.is_alive():
            ...
            if thread.is_alive():
                # 线程仍在运行时不能标记为已停止，否则会导致重复启动
                self.logger.error("服务线程未能在超时时间内停止，保持运行状态")
                return False

        with self._state_lock:
            # 仅在当前线程仍是我们刚刚等待的线程时清理状态
            if self.thread is thread or self.thread is None:
                self._running = False
                self.station = None
                self.thread = None
            else:
                self.logger.warning("检测到新服务线程，跳过停止状态覆盖")
                return False
