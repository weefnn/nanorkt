...
class RTKManager:
    ...
    def start_service(self) -> bool:
        ...
        with self._state_lock:
            ...
            try:
                ...
                self.logger.error(f"RTK 服务线程启动失败: {e}")
                self._running = False
                self.station = None
                self.thread = None
                return False
    ...
    def _run_station(self, station: RTKBaseStation) -> None:
        """
        在线程中运行 RTK 基站
        ...
        try:
            ...
            with self._state_lock:
                # 仅清理当前线程绑定的实例，避免旧线程误清理新实例状态
                if self.station is station:
                    self._running = False
                    self.station = None
                    self.thread = None
