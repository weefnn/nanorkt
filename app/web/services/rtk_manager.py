...
class RTKManager:
    ...
    def _run_station(self, run_id: int, station: RTKBaseStation) -> None:
        """
        在线程中运行 RTK 基站
        ...
        try:
            ...
            with self._state_lock:
                # 仅清理当前代次，避免旧线程覆盖新线程状态
                if self._run_id == run_id and self.station is station:
                    self._running = False
                    self.station = None
                    self.thread = None
    ...
    def stop_service(self) -> bool:
        ...
        if thread and thread.is_alive():
            ...
            if thread.is_alive():
                self.logger.warning("服务线程未能在超时时间内停止，保持运行状态")
                return False
        
        with self._state_lock:
            if self._run_id == run_id and self.thread is thread:
                self._running = False
                self.station = None
                self.thread = None
